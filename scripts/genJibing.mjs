// Generate 健康安全宫位 articles for content/seo/jibing/
// Usage:
//   npx tsx --env-file=.env.local scripts/genJibing.mjs
//   npx tsx --env-file=.env.local scripts/genJibing.mjs --slug ziwei
//   npx tsx --env-file=.env.local scripts/genJibing.mjs --force

import fs from "fs";
import path from "path";
import { JIBING } from "../lib/jibingData.ts";
import { getJibingContent } from "../lib/seoContent.ts";
...
const outDir = path.join(process.cwd(), "content", "seo", "jibing");
...
console.log(`\n⚡ 健康安全宫位 — generating ${entries.length} article(s)${force ? " (--force)" : ""}${dryRun ? " (--dry-run)" : ""}\n`);
...
for (const entry of entries) {
  ...
  try {
    const { markdown, refs } = await getJibingContent(entry, undefined);
