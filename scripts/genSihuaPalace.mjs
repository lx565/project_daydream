...
//   npx tsx --env-file=.env.local scripts/genSihuaPalace.mjs --force

import fs from "fs";
import path from "path";
import { SIHUA_PALACE } from "../lib/sihuaPalaceData.ts";
import { getSihuaPalaceContent } from "../lib/seoContent.ts";
...
const outDir = path.join(process.cwd(), "content", "seo", "sihua-palace");
...
console.log(`\n✦ 四化×十二宮 — generating ${entries.length} article(s)${force ? " (--force)" : ""}${dryRun ? " (--dry-run)" : ""}\n`);
...
for (const entry of entries) {
  ...
  try {
    const { markdown, refs } = await getSihuaPalaceContent(entry, undefined);
