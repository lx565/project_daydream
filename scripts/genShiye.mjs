// Generate 事业宫位 articles for content/seo/shiye/
// Usage:
//   npx tsx --env-file=.env.local scripts/genShiye.mjs
//   npx tsx --env-file=.env.local scripts/genShiye.mjs --slug ziwei
//   npx tsx --env-file=.env.local scripts/genShiye.mjs --force

import fs from "fs";
import path from "path";
import { SHIYE } from "../lib/shiyeData.ts";
import { getShiyeContent } from "../lib/seoContent.ts";
...
const outDir = path.join(process.cwd(), "content", "seo", "shiye");
...
console.log(`\n⚡ 事业宫位 — generating ${entries.length} article(s)${force ? " (--force)" : ""}${dryRun ? " (--dry-run)" : ""}\n`);
...
for (const entry of entries) {
  ...
  try {
    const { markdown, refs } = await getShiyeContent(entry, undefined);
