// Generate 流年运势 articles for content/seo/liunian/
// Usage:
//   npx tsx --env-file=.env.local scripts/genLiuNian.mjs
//   npx tsx --env-file=.env.local scripts/genLiuNian.mjs --slug liunian-jieshao
//   npx tsx --env-file=.env.local scripts/genLiuNian.mjs --kind gongwei
//   npx tsx --env-file=.env.local scripts/genLiuNian.mjs --force

import fs from "fs";
import path from "path";
import { LIUNIAN } from "../lib/liuNianData.ts";
import { getLiuNianContent } from "../lib/seoContent.ts";
...
const outDir = path.join(process.cwd(), "content", "seo", "liunian");
...
console.log(`\n⚡ 流年运势 — generating ${entries.length} article(s)${force ? " (--force)" : ""}${dryRun ? " (--dry-run)" : ""}\n`);
...
for (const entry of entries) {
  ...
  try {
    const { markdown, refs } = await getLiuNianContent(entry, undefined);
