// Generate 四化详解 articles for content/seo/sihua/
// Usage:
//   npx tsx --env-file=.env.local scripts/genSihua.mjs
//   npx tsx --env-file=.env.local scripts/genSihua.mjs --slug hua-ji
//   npx tsx --env-file=.env.local scripts/genSihua.mjs --kind huaji
//   npx tsx --env-file=.env.local scripts/genSihua.mjs --force

import fs from "fs";
import path from "path";
import { SIHUA } from "../lib/sihuaData.ts";
import { getSihuaContent } from "../lib/seoContent.ts";
...
const outDir = path.join(process.cwd(), "content", "seo", "sihua");
...
console.log(`\n✦ 四化详解 — generating ${entries.length} article(s)${force ? " (--force)" : ""}${dryRun ? " (--dry-run)" : ""}\n`);
...
for (const entry of entries) {
  ...
  try {
    const { markdown, refs } = await getSihuaContent(entry, undefined);
