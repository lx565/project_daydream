// Generate 财运宫位 articles for content/seo/caiyun/
// Usage:
//   npx tsx --env-file=.env.local scripts/genCaiyun.mjs
//   npx tsx --env-file=.env.local scripts/genCaiyun.mjs --slug ziwei
//   npx tsx --env-file=.env.local scripts/genCaiyun.mjs --force

import fs from "fs";
import path from "path";
import { CAIYUN } from "../lib/caiyunData.ts";
import { getCaiyunContent } from "../lib/seoContent.ts";
...
const outDir = path.join(process.cwd(), "content", "seo", "caiyun");
...
console.log(`\n⚡ 财运宫位 — generating ${entries.length} article(s)${force ? " (--force)" : ""}${dryRun ? " (--dry-run)" : ""}\n`);
...
for (const entry of entries) {
  ...
  const t = Date.now();
  process.stdout.write(`  GEN   ${entry.name}在财帛宫 (${entry.urlSlug}) … `);

  try {
    const { markdown, refs } = await getCaiyunContent(entry, undefined);
