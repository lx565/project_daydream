...
export const GUIDE_TOPICS: GuideTopic[] = [
  // ── 入門 ─────────────────────────────────────────────────────────────────
  {
    ...
    related: ["十二宮", "主星總覽", "三派比較", "紫微八字區別", "三合派", "紫微斗數準嗎"],
  },
  {
    slug: "十二宮",
    urlSlug: "palaces",
  ...
  },
  {
    ...
    related: ["紫微斗數入門", "合盤", "格局", "紫微八字選擇"],
  },

  // ── 常見疑問 ─────────────────────────────────────────────────────────────
  {
    slug: "紫微斗數準嗎",
    urlSlug: "ziwei-accuracy",
    title: "紫微斗數準嗎？",
    subtitle:
      "紫微斗數不是預言機——它從你的出生時刻推出一張星曜結構圖，用來描述性格傾向與人生格局，不是鐵口直斷的宿命判決。",
    intro:
      "紫微斗數是否『準』，要看你問的是哪一種準確。它不能像天氣預報一樣算出你哪天會發生什麼事，也無法保證考試上榜或投資獲利——任何命理師如此承諾都值得懷疑。它真正做的，是依出生年月日時排出命盤，用十二宮、星曜與四化的結構，描述性格傾向、人生各領域的可能格局與階段性節奏。這套系統歷經數百年整理與驗證，邏輯自洽、可重複推演，但終究是一套符號化的推理框架，不是物理定律。命裡的態度是：如實呈現命盤結構告訴你的東西，不誇大、不嚇唬，最終的判斷與行動仍在你自己手上。",
    ragQuery: {
      text: "紫微斗數 準確性 命理本質 象數 符號系統 性格傾向 命由天定 心可轉運 命理侷限 驗證",
      topic: "格局",
    },
    related: ["紫微斗數入門", "紫微八字選擇", "三派比較", "格局"],
    ctaLabel: "不必先信 · 免費看你的命盤總覽，自己判斷準不準 →",
  },
  {
    slug: "紫微八字選擇",
    urlSlug: "ziwei-vs-bazi-which",
    title: "紫微斗數和八字，哪個更準？該用哪一個？",
    subtitle:
      "兩者不是比賽誰更準，而是兩種角度：紫微斗數畫出宮位地圖，八字算五行氣場——命裡的作法是兩盤合參、互相印證，而不是二選一。",
    intro:
      "很多人一開始會糾結『紫微準還是八字準』，但這個問題背後其實預設了兩者在比賽同一件事——其實不是。八字（子平）以出生年月日時排出四柱，看天干地支的五行生剋與旺衰，長於分析性格底層邏輯與大運起伏的『氣』；紫微斗數以星曜落入十二宮成盤，宮位分工明確、事象具體，長於逐宮細看感情、事業、財運等『象』。兩者是同一組出生資訊的兩種讀法，不是互相競爭的準確度排名，多數認真的命理師會兩者合參、互相印證。命裡的做法正是不強迫你二選一：紫微斗數與八字雙系統推演，交叉比對兩套系統對同一件事的判斷是否一致——一致處更可信，矛盾處會標註出來讓你自行判斷，而不是只挑一套講得好聽的講給你聽。",
    ragQuery: {
      text: "紫微斗數 八字 子平 比較 準確 互補 合參 雙盤 差異 選擇 命理系統",
      topic: "格局",
    },
    related: ["紫微八字區別", "紫微斗數入門", "紫微斗數準嗎", "格局"],
    ctaLabel: "不用選邊站 · 紫微、八字雙系統同時排盤，AI 交叉比對 →",
  },
  {
    slug: "不知道出生時辰",
    urlSlug: "unknown-birth-hour",
    title: "不知道出生時辰怎麼辦？",
    subtitle:
      "時辰決定紫微命宮的位置，但八字的年、月、日三柱不受影響——就算時辰不確定，也能先從八字看起，紫微則建議先想辦法查證。",
    intro:
      "出生時辰在紫微斗數裡地位特殊：它直接決定命宮落在十二宮的哪一個位置，時辰算錯，整張命盤的宮位就全部錯位。相較之下，八字對時辰的依賴較低——時柱只是年、月、日、時四柱之一，就算時辰不確定，年柱、月柱、日柱依然成立，日主旺衰與大運仍可分析，只是少了時柱這一塊拼圖。命裡的出生時辰欄位目前需要選擇一個具體時辰（沒有『不確定』選項）。若你不知道自己的時辰，可以查出生證明、戶口名簿，或聯絡出生醫院調閱病歷，也可以問父母長輩大概的時段（例如天剛亮、吃早飯前後）。如果真的完全無從查起，建議先選最接近的估計時段生成解讀——重點是先動手試一次，看到命盤結構之後，很多人反而更容易回想或查證出正確的時辰。",
    ragQuery: {
      text: "出生時辰 生時不詳 時柱 命宮 定盤 推算時辰 早子時晚子時 時辰重要性 排盤",
      topic: "格局",
    },
    related: ["紫微斗數入門", "十二宮", "紫微八字區別"],
    ctaLabel: "不確定時辰也能先試 · 選最接近的時段，AI 立刻幫你排盤 →",
  },
  {
    slug: "免費付費差別",
    urlSlug: "free-vs-paid",
    title: "免費和付費版本有什麼差別？可以只用免費版嗎？",
    subtitle:
      "免費版看得到命盤總覽與大致方向；完整命書（十二宮逐宮精解、大運流年逐年詳批、八字雙重印證等）是一次性 $6.99、永久解鎖，不是訂閱。",
    intro:
      "命裡目前的免費與付費界線很清楚：輸入出生資訊、生成命盤是完全免費的，你可以先看到命盤總覽——大致的格局方向與基本資訊。如果想看完整命書，內容包含十二宮逐宮精解、大運流年運勢時機的逐年詳批、八字命理雙重印證、三派各自論斷、特別注意的風險與化解，以及可以針對命盤細節持續向 AI 追問，這部分是一次性付費 $6.99（約人民幣50元），付一次永久解鎖、可重複查閱，不是訂閱制，不會自動扣款。合盤功能（紫微合盤、八字合盤）也是同樣邏輯：緣分指數與『緣分一瞥』完全免費，更深入的合盤解讀（飛化互入、合盤三方四正、緣分時機等）才需要另外解鎖。如果你只是想先了解自己命盤的大致方向，免費版已經足夠；想要逐宮、逐年的具體解讀，才需要解鎖完整版。",
    ragQuery: {
      text: "命書 內容 深度解讀 十二宮 逐宮 大運流年 命理服務",
      topic: "格局",
    },
    related: ["紫微斗數入門", "合盤", "格局"],
    ctaLabel: "先看免費總覽 · 想看逐宮逐年詳解，再決定要不要解鎖 →",
  },
] as const;
