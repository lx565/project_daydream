...
import { BAZI_SHIYE, getBaziShiye } from "@/lib/baziShiyeData";
import { getBaziShiyeContent } from "@/lib/seoContent";
...
export async function generateStaticParams() {
  return BAZI_SHIYE.map(e => ({ slug: e.urlSlug }));
}
...
export async function generateMetadata(
  ...
  const entry = getBaziShiye(urlSlug);
  if (!entry) return {};
  ...
  return {
    ...
    openGraph: {
      title,
      description,
      url: `https://www.mingli.study/bazi/shiye/${entry.urlSlug}`,
      siteName: "命裡",
      locale: "zh_CN",
      type: "article",
    },
    alternates: {
      canonical: `https://www.mingli.study/bazi/shiye/${entry.urlSlug}`,
    },
  };
}

export default async function BaziShiyeArticlePage(
  { params }: { params: Promise<PageParams> }
) {
  const { slug: rawSlug } = await params;
  const urlSlug = decodeURIComponent(rawSlug);
  ...
  const { markdown, refs } = await getBaziShiyeContent(entry);
  const hasContent = markdown.trim().length > 0;
  const relatedEntries = entry.related
    .map(slug => getBaziShiye(slug))
    .filter((e): e is NonNullable<typeof e> => Boolean(e));
  const pagePath = `/bazi/shiye/${entry.urlSlug}`;
      ...
      <main className="min-h-screen bg-parchment">
        ...
        <div className="max-w-2xl mx-auto px-4 pb-16 space-y-6">
          ...
          <div className="text-center pt-8 pb-4 space-y-2">
            <p className="text-xs text-sky-600 tracking-widest font-medium">八字看事業</p>
            <h1
              className="text-3xl font-bold text-sky-700 leading-snug"
            ...
            <div className="flex items-center gap-3 justify-center pt-1">
              <div className="h-px w-16 bg-sky-300/50" />
          ...
          <ToolCTA variant="card" sub="AI 依據子平命理典籍，結合你的用神五行、官殺格與大運流年，分析適合的行業、事業格局與高峰時段。" label="解析我的八字事業" />
              ...
              <div className="grid grid-cols-2 gap-2">
                {relatedEntries.map(e => (
                  <Link
                    ...
                    className="paper-card rounded-xl border border-border-warm p-3 text-sm text-ink-2 hover:border-sky-300 hover:text-sky-700 transition-colors"
                  >
          ...
          <ToolCTA variant="slim" label="八字 + 紫微雙系統 · AI 依據逾百部典籍為你詳批事業 →" />
