...
import { BAZI_CAIYUN, getBaziCaiyun } from "@/lib/baziCaiyunData";
import { getBaziCaiyunContent } from "@/lib/seoContent";
...
export async function generateStaticParams() {
  return BAZI_CAIYUN.map(e => ({ slug: e.urlSlug }));
}
...
export async function generateMetadata(
  ...
  const entry = getBaziCaiyun(urlSlug);
  if (!entry) return {};
  ...
  return {
    ...
    openGraph: {
      title,
      description,
      url: `https://www.mingli.study/bazi/caiyun/${entry.urlSlug}`,
      siteName: "命裡",
      locale: "zh_CN",
      type: "article",
    },
    alternates: {
      canonical: `https://www.mingli.study/bazi/caiyun/${entry.urlSlug}`,
    },
  };
}

export default async function BaziCaiyunArticlePage(
  { params }: { params: Promise<PageParams> }
) {
  const { slug: rawSlug } = await params;
  const urlSlug = decodeURIComponent(rawSlug);
  ...
  const { markdown, refs } = await getBaziCaiyunContent(entry);
  const hasContent = markdown.trim().length > 0;
  const relatedEntries = entry.related
    .map(slug => getBaziCaiyun(slug))
    .filter((e): e is NonNullable<typeof e> => Boolean(e));
  const pagePath = `/bazi/caiyun/${entry.urlSlug}`;
      ...
      <main className="min-h-screen bg-parchment">
        ...
        <div className="max-w-2xl mx-auto px-4 pb-16 space-y-6">
          ...
          <div className="text-center pt-8 pb-4 space-y-2">
            <p className="text-xs text-amber-600 tracking-widest font-medium">八字看財運</p>
            <h1
          ...
          <ToolCTA variant="card" sub="AI 依據子平命理典籍，結合你的財格、身財關係與大運流年，分析求財方式、發財時機與守財之道。" label="解析我的八字財運" />
          ...
          <ToolCTA variant="slim" label="八字 + 紫微雙系統 · AI 依據逾百部典籍為你詳批財運 →" />
