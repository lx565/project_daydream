...
export async function generateStaticParams() {
  return BAZI_JIBING.map(e => ({ slug: e.urlSlug }));
}
...
export async function generateMetadata(
  ...
  const entry = getBaziJibing(urlSlug);
  if (!entry) return {};
  ...
  return {
    ...
    openGraph: {
      title,
      description,
      url: `https://www.mingli.study/bazi/jibing/${entry.urlSlug}`,
      siteName: "命裡",
      locale: "zh_CN",
      type: "article",
    },
    alternates: {
      canonical: `https://www.mingli.study/bazi/jibing/${entry.urlSlug}`,
    },
  };
}

export default async function BaziJibingArticlePage(
  { params }: { params: Promise<PageParams> }
) {
  const { slug: rawSlug } = await params;
  const urlSlug = decodeURIComponent(rawSlug);
  ...
  const { markdown, refs } = await getBaziJibingContent(entry);
  const hasContent = markdown.trim().length > 0;
  const relatedEntries = entry.related
    .map(slug => getBaziJibing(slug))
    .filter((e): e is NonNullable<typeof e> => Boolean(e));
  const pagePath = `/bazi/jibing/${entry.urlSlug}`;
      ...
      <main className="min-h-screen bg-parchment">
        ...
        <div className="max-w-2xl mx-auto px-4 pb-16 space-y-6">
          ...
          <div className="text-center pt-8 pb-4 space-y-2">
            <p className="text-xs text-emerald-600 tracking-widest font-medium">八字看健康安全</p>
            <h1
              className="text-3xl font-bold text-emerald-700 leading-snug"
          ...
          {refs.length > 0 && (
            <div className="space-y-2">
              ...
              <p className="text-[11px] text-ink-4/80 leading-relaxed">
                本文由命裡 AI 綜合典籍整理，僅供學習參考，非醫學診斷，如有不適請及時就醫。
              </p>
            </div>
          )}
          ...
          <ToolCTA variant="card" sub="AI 依據子平與五行養生理路，結合你的五行強弱與用神，分析先天體質傾向與可落地的養生方向。僅供保健參考，非醫學診斷。" label="解析我的八字型質" />
              ...
              <div className="grid grid-cols-2 gap-2">
                {relatedEntries.map(e => (
                  <Link
                    ...
                    className="paper-card rounded-xl border border-border-warm p-3 text-sm text-ink-2 hover:border-emerald-300 hover:text-emerald-700 transition-colors"
                  >
          ...
          <ToolCTA variant="slim" label="八字 + 紫微雙系統 · AI 依據逾百部典籍為你詳批體質健康 →" />
