...
import { TIANGAN, getTiangan } from "@/lib/baziTiangan";
import { getTianganContent } from "@/lib/seoContent";
...
export async function generateStaticParams() {
  return TIANGAN.map(t => ({ slug: t.urlSlug }));
}
...
export async function generateMetadata(
  ...
  const entry = getTiangan(urlSlug);
  if (!entry) return {};

  const title = `${entry.gan}${entry.element}日主是什麼樣的人？八字十天干詳解 — 命裡`;
  ...
  return {
    ...
    openGraph: {
      title,
      description,
      url: `https://www.mingli.study/bazi/tiangan/${entry.urlSlug}`,
      siteName: "命裡",
      locale: "zh_CN",
      type: "article",
    },
    alternates: {
      canonical: `https://www.mingli.study/bazi/tiangan/${entry.urlSlug}`,
    },
  };
}

export default async function TianganPage(
  { params }: { params: Promise<PageParams> }
) {
  const { slug: rawSlug } = await params;
  const urlSlug = decodeURIComponent(rawSlug);
  ...
  const { markdown, refs } = await getTianganContent(entry);
  const hasContent = markdown.trim().length > 0;
  const relatedEntries = entry.related
    .map(slug => getTiangan(slug))
    .filter((s): s is NonNullable<typeof s> => Boolean(s));
  const path = `/bazi/tiangan/${entry.urlSlug}`;

  const faq = seoFaqItems({ name: `${entry.gan}${entry.element}日主`, oneLine: entry.oneLine, intro: entry.intro });
    ...
    <>
      <JsonLd data={[
        breadcrumbSchema([
          ...
          { name: "八字", path: "/bazi" },
          { name: `${entry.gan}${entry.element}日主`, path },
        ]),
        articleSchema({
          headline: `${entry.gan}${entry.element}日主詳解 · 八字十天干`,
      ...
      <main className="min-h-screen bg-parchment">
        <LibraryNav category="bazi" currentTitle={`${entry.gan}${entry.element}日主`} />

        <div className="max-w-2xl mx-auto px-4 pb-16 space-y-6">

          {/* Hero */}
          <div className="text-center pt-8 pb-4 space-y-2">
            <p className="text-xs text-ink-4 tracking-widest">八字日主 · {entry.yinyang}{entry.element} · {entry.image}</p>
            <h1
              className="text-4xl font-bold text-gold leading-snug"
              style={{ fontFamily: "var(--font-serif)", letterSpacing: "0.1em" }}
            >
              {entry.gan}{entry.element}日主
            </h1>
          ...
          <ToolCTA variant="slim" label="排你的八字 · 看你是什麼日主、喜用什麼五行 →" />
          ...
          {relatedEntries.length > 0 && (
            <div className="space-y-3">
              <p className="text-xs text-ink-4 font-medium">相關日主</p>
              <div className="grid grid-cols-2 gap-2">
                {relatedEntries.map(s => (
                  <Link
                    key={s.urlSlug}
                    href={`/bazi/tiangan/${s.urlSlug}`}
