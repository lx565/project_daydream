...
import { GEJU, getGeju } from "@/lib/baziGeju";
import { getGejuContent } from "@/lib/seoContent";
...
export async function generateStaticParams() {
  return GEJU.map(g => ({ slug: g.urlSlug }));
}
...
export async function generateMetadata(
  ...
  const entry = getGeju(urlSlug);
  if (!entry) return {};

  const title = `${entry.name}是什麼？成格、破格與喜忌詳解 — 命裡`;
  ...
  return {
    ...
    openGraph: {
      title,
      description,
      url: `https://www.mingli.study/bazi/geju/${entry.urlSlug}`,
      siteName: "命裡",
      locale: "zh_CN",
      type: "article",
    },
    alternates: {
      canonical: `https://www.mingli.study/bazi/geju/${entry.urlSlug}`,
    },
  };
}

export default async function GejuPage(
  { params }: { params: Promise<PageParams> }
) {
  const { slug: rawSlug } = await params;
  const urlSlug = decodeURIComponent(rawSlug);
  ...
  const { markdown, refs } = await getGejuContent(entry);
  const hasContent = markdown.trim().length > 0;
  const relatedEntries = entry.related
    .map(slug => getGeju(slug))
    .filter((s): s is NonNullable<typeof s> => Boolean(s));
  const path = `/bazi/geju/${entry.urlSlug}`;
      ...
      <main className="min-h-screen bg-parchment">
        ...
        <div className="max-w-2xl mx-auto px-4 pb-16 space-y-6">
          ...
          <div className="text-center pt-8 pb-4 space-y-2">
            <p className="text-xs text-ink-4 tracking-widest">八字格局 · {entry.kind}</p>
            <h1
          ...
          <ToolCTA variant="slim" label="排你的八字 · 看你的命局成什麼格、是否成格 →" />
          ...
          <ToolCTA variant="card" sub="格局只是骨架，還要結合日主旺衰與大運流年。AI 依據子平、調候、格局多派典籍為你詳批。" label="生成我的八字詳批" />
          ...
          {relatedEntries.length > 0 && (
            <div className="space-y-3">
              <p className="text-xs text-ink-4 font-medium">相關格局</p>
              <div className="grid grid-cols-2 gap-2">
                {relatedEntries.map(s => (
                  <Link
                    key={s.urlSlug}
                    href={`/bazi/geju/${s.urlSlug}`}
