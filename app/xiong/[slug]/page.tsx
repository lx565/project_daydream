...
import { XIONG, getXiong } from "@/lib/xiongData";
import { getXiongContent } from "@/lib/seoContent";
...
export async function generateStaticParams() {
  return XIONG.map(e => ({ slug: e.urlSlug }));
}
...
export async function generateMetadata(
  ...
  return {
    ...
    openGraph: {
      title,
      description,
      url: `https://www.mingli.study/xiong/${entry.urlSlug}`,
      siteName: "命裡",
      locale: "zh_TW",
      type: "article",
    },
    alternates: {
      canonical: `https://www.mingli.study/xiong/${entry.urlSlug}`,
    },
  };
}
...
export default async function XiongArticlePage(
  { params }: { params: Promise<PageParams> }
) {
  const { slug: rawSlug } = await params;
  const urlSlug = decodeURIComponent(rawSlug);
  ...
  const { markdown, refs } = await getXiongContent(entry);
  const hasContent = markdown.trim().length > 0;
  const relatedEntries = entry.related
    .map(slug => getXiong(slug))
    .filter((e): e is NonNullable<typeof e> => Boolean(e));
  const pagePath = `/xiong/${entry.urlSlug}`;
  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-parchment">
        <LibraryNav category="xiong" currentTitle={entry.name} />

        <div className="max-w-2xl mx-auto px-4 pb-16 space-y-6">

          {/* Hero */}
          <div className="text-center pt-8 pb-4 space-y-2">
            <p className="text-xs text-slate-500 tracking-widest font-medium">
              {KIND_LABELS[entry.kind]}
            </p>
            <h1
              className="text-3xl font-bold text-slate-700 leading-snug"
            ...
            <div className="flex items-center gap-3 justify-center pt-1">
              <div className="h-px w-16 bg-slate-300/50" />
          ...
          <ToolCTA variant="card" sub="AI 依據逾百部典籍，結合你的命盤格局分析煞星、空亡與兇格的具體影響與應對方向。" label="解析我的命盤難點" />
              ...
              <div className="grid grid-cols-2 gap-2">
                {relatedEntries.map(e => (
                  <Link
                    ...
                    className="paper-card rounded-xl border border-border-warm p-3 text-sm text-ink-2 hover:border-slate-300 hover:text-slate-700 transition-colors"
                  >
          ...
          <ToolCTA variant="slim" label="紫微斗數 AI · 依據逾百部典籍為你詳批命盤格局 →" />
