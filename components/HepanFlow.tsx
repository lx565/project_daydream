...
function HepanResult({ charts, onReset }: { charts: Charts; onReset: () => void }) {
  ...
  // Full reading — only stream when not gated (never pay for a locked section).
  const full = useSSEStream("/api/reading/couple", `${coupleChartId}_full`);
  useEffect(() => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <p className="text-xs text-ink-4 tracking-widest uppercase mb-2 px-1 flex items-center gap-2">
          <span className="w-px h-3 bg-vermillion inline-block" />
          <span className="text-vermillion">完整合盤解讀</span>
        </p>
        {paywall.loading ? (
          <div className="paper-card rounded-2xl border border-border-warm p-4 sm:p-5"><LoadingSkeleton /></div>
        ) : gated ? (
          <PaywallLock chartId={coupleChartId} sectionLabel="雙人合盤 · 完整解讀" included={COUPLE_INCLUDED} />
        ) : (
          <div className="paper-card rounded-2xl border border-border-warm p-4 sm:p-5">
